...
A chip is an inline atom in real text flow that owns its gap as a symmetric 2px margin: two chips that touch in the document sit 4px apart, the caret between them lands where their margins meet, and the same holds in a right-to-left line. The line keeps no space on a chip's behalf, ever: the spaces beside a chip are the user's, typed or not, the way a mention behaves in any text field. The browser does the rest natively (one press crosses a chip, Backspace flush after one removes it); the single hand-written key rule is the position the browser drops, a caret anchored on the line between two touching chips, and it lives on `beforeinput` so a phone's keyboard reaches it. At a line position beside a chip (between two, before the first, after the last) no browser paints the caret well, so on a fine pointer one is drawn there at the pill's height, centred in the gap, the way atom editors draw a gap cursor; in text the browser's caret is the caret. On touch the platform's caret and word-snap stand, and a tap is corrected only in the line's padding. A Backspace or Delete with nothing on its side at a line edge is swallowed, because both engines otherwise clear the line. `composer/line/invariants.ts` keeps the browser's untidiness out (merged runs, no empty nodes); `render.ts` drops the seams older briefs stored.
