...
- **The picture zooms where it is.** Its row is the viewport and its layout is the fit, so at fit nothing is transformed. The wheel, with or without ctrl, a trackpad pinch or two fingers zoom about the cursor, and the picture stays under the cursor rather than snapping back to centre; a drag or a finger pans; a double click or double tap goes to actual size and back; plus, minus, 0 and 1 do the same from the keyboard. The header reads Fit, Fill or a percent, where 100% is the picture at its own pixel size in CSS pixels (the size the browser gives an image it is not asked to resize, and never smaller than fit), and its menu names the stops. The range ends at 300%. No modal viewer: inspection and refinement are one surface, and the strip and the panel never move under a zoom.
- **The picture answers a right click.** A right click or a long press on it opens the shot's own verbs, the ones the header carries, then the zoom stops, the way a tile in the feed does; the browser's menu never shows over a shot.
