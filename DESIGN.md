...
A person is established visually, in the create dialog (`?new=presenter`, `.sc-pstudio`): a wide card, the stage on the left and a narrow rail on the right; on a phone the same pieces stack into one sheet with one scroller and a bottom that stays put. Two ways to start, as tabs in the tab grammar, never pills: From scratch (one sentence, three tappable examples, Create person) and From photos (one to four photographs, the likeness line, Continue). Then one face, drawn and decided: Use this person, Try again, or a sentence that adjusts it. Then the full body and the three-quarter view, each drawn from the views used before it, decided in turn with Use or Try again, the next one starting on its own. The strip under the stage is the progress and the navigation: three tiles, the current one outlined, a mark on what stands, no step numbers. The composer at the bottom of the rail is an editing field, not a chat: a sentence changes the person (the face is redrawn and the other views follow once it is used) or the view on the stage; a chip in the card names the picture Refine will redraw and follows the sentence as it is typed, and the line under the card says what that means, both before Refine; a revision offers Use or Keep previous; a photograph is never redrawn. The title reads Create presenter until the three views stand, Refine presenter after; while nothing is under the well the empty well takes the stage's full height. The name comes last, with Details behind one quiet button. Closing keeps the draft; Start over is the quiet `Confirm`. No published state, no assistant prose, no engine call spent on words. Without an engine, From scratch shows the setup card and From photos saves the photographs as they are.
