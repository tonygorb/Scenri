...
A chip is an inline atom in real text flow that owns its gap as a symmetric 2px margin: two chips that touch in the document sit 4px apart, the caret between them lands where their margins meet, and the same holds in a right-to-left line. The line keeps no space on a chip's behalf, ever: the spaces beside a chip are the user's, typed or not, the way a mention behaves in any text field. The browser does the rest natively (one press crosses a chip, Backspace flush after one removes it); the single hand-written key rule is the position the browser drops, a caret anchored on the line between two touching chips, and it lives on `beforeinput` so a phone's keyboard reaches it. On a fine pointer nothing is drawn over the caret; on touch the platform's caret and word-snap stand, and a tap is corrected only in the line's padding. `composer/line/invariants.ts` keeps the browser's untidiness out (merged runs, no empty nodes); `render.ts` drops the seams older briefs stored.
