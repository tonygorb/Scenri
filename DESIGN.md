...
- **Tasks begin where they happen, never after each other.** From Learn, or for someone new the first time they open the presenter studio, a scene or product dialog, or use an open shot's composer (opening a shot is not asking to refine it). A surface begins its task at most once per visit. A task left on another surface gives way to the next; the first shot never does.
...
Settings (`views/SettingsDialog.tsx`) is a detour, not a destination: a dialog over the work, in Learn's shell (`DialogSheet`), 880 wide and one height whatever the page, `min(720px, 100dvh - 48px)`, so choosing another page never moves an edge (approved 2026-09-22). The head carries **Settings** and the X across the whole width on a hairline; the index's hairline runs from it to the floor, and one 24px gutter runs down the surface. **Six pages under two plain labels**: **This brand** (Brand kit, Usage) and **Studio** (Providers, General, About), with **Danger zone** apart from both at the foot of the index, because it holds every delete and this brand's delete is one of them, first, never at the foot of the page where the brand is edited. No brand mark or name in the index: the scope is words. Index rows are 40px, a 16px glyph and the name at 13.5/500 in `--sc-fg2`, the chosen one on the chip fill with no weight change. A page opens with its name at 20/600 and one sentence, then its sections, each titled at 15/600 over a 12.5px sentence, so page, section and row are three sizes rather than one size at three weights. One gutter runs down the surface, as in Learn: the title, the index's words and glyphs, and the page stand on it, and a chosen row's fill bleeds 10 past it; on a phone the title, the scope labels and the cards share the gutter and the close glyph ends where the cards do. The caps sit under the providers they cap and the theme beside the library in General; the old pane ids (`budget`, `appearance`, `library`) still land. **A group is a card**: in light, the panel inside a 1px `--sc-line` hairline at the large radius, never a fill, because `--sc-raised` is the on-state fill and a section painted with it read as selected, with a white hole cut for every field, a chosen index row ten steps from its neighbours and provider tiles gone to outlines. In dark, which separates by lightness, the card rises one step to `--sc-raised` and keeps its hairline: the page, the card and a field had been one colour told apart by one line colour, a wireframe, and lifted, a field reads as a well sunk into the card and a filled action as raised off it; whatever inside drew with `--sc-raised` (a meter's track, an empty day, a provider with no plate) sinks to the panel's colour with the fields. Rows are 56 tall at least, the label at 13.5/500 over a 12px sentence held to 52ch, one control on the right at 32 whatever it is (button, field, select, the palette's remove), and the hairline between rows starts where the words do; fields in one card share one width so their edges line up. The palette is rows like any other card, its colours circles. **What you press is solid, what you type into is drawn**: in a card, whose own edge and hairlines are outlines already, a row's action is filled with `--sc-chip` (the chip ladder on hover and press) and a delete with a 10% red wash, while fields stay outlined wells; a ghost button there read as more of the card (2026-09-22). On a phone a row stacks: the label 8px over its field, the field at the app's 34. The kit writes as it is edited, so there is no Save button: its page head says what the writing is doing, **Saved** for a moment after a write and a failure for as long as it lasts. On a phone it is the studio's sheet, as Learn is: the index first, one card per scope and Danger zone its own, then a page over it with Back; a phone sent to a page by a link opens on that page. **Provider setup** (`views/ProviderSetup.tsx`), which opens from Settings and over it, wears the same shell and Learn's lesson anatomy: the provider's own tile at 44 with its name at 20/600 and where it stands under it (a green dot for ready, a spinner while checking), one 13.5 sentence in the secondary ink, then Learn's own step rows, where the step in hand is the one pressable card carrying its verb as the ink pill, a taken step is a green tick that stands back and a step to come recedes (never gold); what is left to do sits under the steps, the primary in ink and anything beside it filled, a way out as a link; on a phone it takes Settings' own height and replaces its sheet rather than stacking a short one on it. A Radix spinner inside any sheet gets its size from tokens bridged onto the sheet's layer, since the sheet is portalled outside the Radix root.
