...
- **The welcome** (`views/WelcomeDialog.tsx`) is the one interruption, once, for someone new, the first time Home or Create is ready, rested and idle, held behind the tutor's own curtain (`DialogSheet tone="guide"`): one product shot in three worlds, **Welcome to *Scenri***, what the first shot will ask of this install, and **Make your first shot** or **Not now**.
...
- **The help button** (`layout/HelpMenu.tsx`): *First steps*, *Learn*, *Keyboard shortcuts* (Create), *What's new*, *Set up image generation* (only while nothing can generate), *About Scenri*, *Scenri on GitHub*.
