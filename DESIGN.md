...
- **Only the one action.** An ask leaves exactly what it asks for usable: the add control, or the picker's shelf, or the brief and Generate together. Not the search beside the shelf, not the picker's own tabs (there are none while it asks), not the page behind. The card itself carries no work: **Back** takes the last chip out, shows the ask that put it there and opens the shelf on that kind again, so one press puts the choice back in front of them; where nothing can be taken back there is no Back to press. While the tutor is walking someone through a brief a chip can be changed but not removed, by any means: its remove control is hidden and every deletion that would reach a chip, Backspace and Delete and a select-all included, is refused, because Back is the one way anything leaves the brief and it leaves exactly one step. The words are still theirs to write and delete. **X** ends the guidance and leaves every chip and word where they are, **Done** closes the last note. The one exception is the opening, which has nothing to do yet and so carries **Start**.
...
- **The spotlight** (`layout/coachGeometry.ts`): the curtain is `rgba(0,0,0,.52)` under a static `blur(3px)`, only its opacity fades in; `.62` without blur where backdrop-filter is unsupported or transparency is reduced. It is deliberately light: at `.66` and a 6px blur the lit surface stopped reading as part of the page and started to look like a thing floating over it. The lit surface is the shape the asked control sits in, cut at its own edge and corners, so a control is never a hole in fog; what a live control opens (its popover, the phone's settings sheet) joins it while it is open, and a surface that opened out of another keeps that other lit beside it, so the composer never goes dark under its own picker. Inside that surface everything not usable recedes to a third of its strength, and the asked control wears a 2px ring 4px clear of its edge. A card for one control sits against that control, so its pointer lands on the thing being asked for; a card for a whole surface (the picker, a dialog, a question, the brief beside Generate) clears that surface so it covers none of it.
- **On a phone the card is a sheet.** Below 768px it leaves the target and docks to the bottom edge, full width inside a 12px gutter, because a tooltip is a pointer pattern and a touch screen has no pointer: Apple presents a popover as a sheet in the compact size class, Material calls the modal bottom sheet the small-screen pattern, and NN/g is plain that tooltips are not a touch pattern. The ring and the cut-out stay on the target where they are, and the sheet sits above whatever is lit in the lower half of the screen (the composer, the open picker) rather than over it, so the control or the field being asked for is never behind the words (WCAG 2.4.11). It rides the visual viewport, so a raised keyboard pushes it up instead of hiding it; its buttons are at least 44px with the close always in reach (WCAG 2.5.5); and it clears the home indicator with `env(safe-area-inset-bottom)`. It still says which thing it means: the pointer hangs off its bottom edge over the lit surface below.
- **The other tasks are quieter.** Refining: one ask on the open shot's composer (**Change one thing**), quiet while it draws, a note on the new version. A scene: one ask on the dialog it is made in, then quiet until the brand holds one more. The presenter studio asks its own questions one at a time, so the tutor says nothing through them: three decisions earn a word, because the questions do not explain them (**Describe someone, or start from photos**, **Decide the face**, **Save your presenter**), and a wall earns a fourth, because a wall is not a question (**Set up image generation**, when nothing can draw a face). A task that makes something ends when the brand holds one more than when it began; closing its surface does not end it, and a presenter left with a draft is continued, that exact draft, from First steps.
- **Tasks begin where they happen, never after each other.** From First steps, or for someone new the first time they open the presenter studio, a scene dialog, or use an open shot's composer (opening a shot is not asking to refine it). A surface begins its task at most once per visit. A task left on another surface gives way to the next; the first shot never does.
- **First steps** (`views/home/FirstSteps.tsx`) is a plain checklist under Home's greeting: **First steps**, a quiet `2 of 4`, a bare X, then four rows, each launching its task where it happens: **Make your first shot**, **Create a presenter**, **Build a scene**, **Refine a shot**. A to-do is a hollow ring, the one in hand a dot with a **Continue** title, a done step a thin tick. No percentage, no accent colour, no celebration. A step is ticked by what the library holds and latched, so deleting the thing never takes the tick back. Someone new sees it from the start; someone upgrading only after asking. The X hides it with an Undo, Help brings it back, and it leaves when all four are done.
