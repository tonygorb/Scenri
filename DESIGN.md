...
- **Keeping is the composer's to say, once.** A refinement borrows the identity of the shot it refines (product, person, mark, reference) so the engine keeps them; the band over the refine field lists them under one word, `Keeping`, and each chip's x stops keeping it, which leaves it out of the request by key (`drop`) and out of the record. Said forward, never as "carried over": that read as a fact about this shot, and on the Original there was nothing carried at all. The record under BRIEF is the ask alone; the same list there under another meaning was two lists with two meanings on one screen. The world is never a chip: it is kept through the photograph and is the Original's to say.
