...
- **Split:** one primary control with two hit regions, for a verb that has a usual thing and a short list of neighbours (the top bar's New). The filled half performs the usual thing, and leads with a 28px disc in the fill's own ink carrying the plus in the fill's colour (black on the white pill, white on the black), inset 2px so the disc and the round end share a centre, then the word at 13.5/500; a 1px divider in the fill's own ink at 22%; the caret half opens the menu. Its shape is the primary's full round, so each end is a half-circle at any height, and each half carries that curve on its own end so a focus ring follows the shape. Its press is the one sanctioned exception to the primary's opacity step: a shade of the fill (10% hover, 18% pressed, of `--sc-inv-fg` over `--sc-inv-bg`) that covers the whole control over the plus and the word, since the whole control is that act, and only the caret's own half over the caret, since that press opens the menu instead. An opacity step cannot say which of the two a press will be, and saying that is the control's whole purpose. Never for a verb with no usual thing: that is a menu wearing a loud coat.
...
### First use: a tutor, and First steps
...
- **Only the one action.** An ask leaves exactly what it asks for usable: the add control, or the picker's shelf, or the brief and Generate together. Not the search beside the shelf, not the picker's own tabs (there are none while it asks), not its way to make a new one (Add product, Create presenter, Upload image are left out while it asks: a presenter made from there is a whole studio away), not the page behind. What else answers the same question stays usable too, never waited for and never handed the caret: the studio's own line where a typed sentence is the answer, the portrait beside the question about it. The card itself carries no work: **Back** takes the last chip out, shows the ask that put it there and opens the shelf on that kind again, so one press puts the choice back in front of them; where nothing can be taken back there is no Back to press. While the tutor is walking someone through the first shot's brief a chip can be changed but not removed, by any means: its remove control is hidden, its own panel offers no Remove, and every deletion that would reach a chip, Backspace and Delete and a select-all included, is refused, because Back is the one way anything leaves the brief and it leaves exactly one step. The same walk has one way in: `$`, `@`, `/` and `#` are characters while it is on, so every chip arrives through the ask for it. Someone who already knows them closes the guide and has them back. Only that brief is held: an open shot's composer and the Home dock never are. The words are still theirs to write and delete. **X** ends the guidance and leaves every chip and word where they are, **Done** closes the last note. The one exception is the opening, which has nothing to do yet and so carries **Start**.
...
- **The other tasks are quieter.** Refining: one ask on the open shot's composer (**Change one thing**) with the picture itself kept in sight, because saying what to change means looking at it; quiet while it draws; a note on the history strip where the change landed (**Here is the change**, never "version"); a change that fails is said on the history too (**That change didn't work**) until they step back to a shot with a composer. A product: one ask on its dialog about the one thing the dialog does not explain, where its photos come from (**Add your product**), then quiet until the brand holds one more. A scene is made in its studio, which asks its own questions, so the tutor says only what they do not: what a scene is, at the first question (**Describe the place, or start from pictures**), and at the read-back that those words are the scene (**These words are the scene**), then quiet until the brand holds one more. The presenter studio asks its own questions one at a time, so the tutor says nothing through them: three decisions earn a word, because the questions do not explain them (**Describe someone, or start from photos**, **Decide the face**, **Save your presenter**), each leaving the studio's own line usable where a typed sentence answers it (the description; the adjustment to a face), with the face itself in sight: the portrait on the stage, and on a phone, which has no stage, the conversation's own picture of it; and a wall earns a fourth, because a wall is not a question (**Set up image generation**, when nothing can draw a face). A task that makes something ends when the brand holds one more than when it began; closing its surface does not end it, and a presenter left with a draft is continued, that exact draft, from First steps.
- **Tasks begin where they happen, never after each other.** From First steps or Learn, or for someone new the first time they open the presenter studio, a scene or product dialog, or use an open shot's composer (opening a shot is not asking to refine it). A surface begins its task at most once per visit. A task left on another surface gives way to the next; the first shot never does.
- **First steps** (`views/home/FirstSteps.tsx`) is a plain checklist under Home's greeting: **First steps**, a quiet `2 of 4`, a quiet **All lessons** that opens Learn, a bare X, then four rows, each launching its task where it happens: **Make your first shot**, **Create a presenter**, **Build a scene**, **Refine a shot**. A to-do is a hollow ring, the one in hand a dot with a **Continue** title, a done step a thin tick. No percentage, no accent colour, no celebration. A step is ticked by what the library holds and latched, so deleting the thing never takes the tick back. Someone new sees it from the start; someone upgrading only after asking. The X hides it with an Undo, Help brings it back, and it leaves when all four are done.
...
- **Learn** (`views/LearnDialog.tsx`, the list in `lessons.ts`) is every lesson, for whenever someone wants one, where First steps is a new install's short list. A lesson is a guided task with a few words about it, never an article or a video: **Make your first shot**, **Add your product**, **Create a presenter**, **Build a scene**, **Refine a shot**. `lessons.ts` is the one list; First steps shows the four marked `firstStep`, Learn shows all five, and both read done, in hand or new from the same install record, so a lesson done from either (or done without the tutor at all) is done in both. It is a dialog in the create dialogs' shell and lives in the address like Settings: `?learn=lessons` for every lesson, `?learn=<task>` for one. Every lesson is a picture of what it makes (the shipped catalog, never an illustration drawn for it), its name and where it stands in a few words: `4 steps`, `Step 2 of 4`, or a tick and `Done`. Never a time, because a generation takes as long as it takes, and never a colour or a percentage. One lesson is its picture, one sentence of what it gives you, its steps as outcomes (never clicks) marked with First steps' own ring, dot and tick, and one action: **Start**, **Continue**, **Do it again**, or, for refining with no shot yet, **Make a shot first**. A step is ticked only when the product proves it. Starting closes Learn and hands over to the tutor where the task happens. On a desktop the two levels share one box so moving between them never moves the card; on a phone it is the sheet, every lesson a row with its picture leading, and the same one height for both levels.
- **Learn is help, not a place.** The top bar's destinations are five, Home, Create, Products, Presenters and Scenes, because a nav slot has to earn itself against work done every session, and learning is not that. Learn is reached from Help and from First steps' **All lessons**, and never gets a sixth slot.
- **The help button** (`layout/HelpMenu.tsx`): *First steps*, *Learn*, *Welcome to Scenri*, *Keyboard shortcuts* (Create), *What's new*, *Set up image generation* (only while nothing can generate), *About Scenri*, *Scenri on GitHub*.
...
A scene is the place and its light around a shot, and a shot is told it in words. It is made in the presenter's studio (the same frame, transcript, composer and stage, `.sc-pstudio[data-kind="scene"]`, at `/scenes/new` and `/scenes/:sceneId/edit`, a child of the library and of the scene's page), asking a place's questions and never a person's. The first question has two doors, pictures of the place or a few questions, and a sentence typed there is the place itself. The questions are four rows, each skippable and each answerable in words: the world it is, whether to keep the light that world already has, how the subject sits in it, and where the camera stands. They were five (place, light, feeling, material, figure), which is a taxonomy asked before anything is on screen; nobody pictures a shot from an adjective, and "Warm" or "Wood" made the decision harder. A world is a starting direction, not the final picture: eight worlds each hold the same plain unbranded bottle so they are compared rather than admired, six lights in one unchanged plaster room, five cameras around one unchanged sweep. Staging is a choice in words, because it is a decision about the subject rather than another row of pictures. A world passed over its light keeps the light its own card was photographed in. A staging pick that already is a camera (hands, small in the space) fills that row so it is not asked again. A phrase typed at the first question answers the rows it names and only the rest are asked; a whole description still skips all of them. Passing every row is allowed and spends no reading: there is nothing to read, so the line stays open and says so. Once the place is given it is read, the one thing the studio does on its own, and read back as **What your shots are told**, the words every shot will get, with one **Draw the scene** under it and the line open for anything to add or leave out; nothing is drawn without that press. The picture lands with one decision, **Use this scene**, **Try again** (the same words, a new picture) or **Change something** (a sentence that revises only what it names and changes the picture from the one before), and every version is a pair of words and picture that **Put back** restores whole. The name is asked while the first picture draws, with the reader's suggestion as a starter, and a sentence that names it ("call it Stone Hall") names it wherever it is said and draws nothing. The conversation reads forward here too: an answer the picture was drawn from asks once before its pencil opens it, because changing it takes back the pictures with everything else asked after it, and the Confirm says the change line is the way to keep the picture and alter one thing. A version is the candidate until Use; nothing is saved before it and nothing rejected reaches Create. A sentence asking to cast a presenter or place a product is answered in one line (they belong to Create), and a scene's pictures are read into words, never sent with a shot, except a figure-led scene's preview, which is the plate a shot beside a presenter conditions on. A scene's own page is a record, and wears the same page a presenter and a product wear: the centred identity (the name, what it is in one sentence, and what it is made of in a few words read off the scene's own `keywords`, which no other surface shows), one verb, then the pictures. A scene with ways to shoot it keeps them **on** that verb as a split button rather than in a band of their own: a way is how Use in a shot is pressed, and a way is written, renamed or taken away in Details, never on the page. The pictures are the middle zone and the only thing allowed to leave the column: your own scene's one drawn picture at the size it was drawn, a curated scene's examples standing side by side in the same rail a presenter's views use. Then what a shot made here is told, as the three keys that are real (Light, Camera, Built around) with one sentence under them saying that a shot's own words win, which is what the compiler actually does. **The set prose is not on this page**: it is eight hundred characters written by the reading, it is changed by re-reading the place, and it belongs to the studio that writes it, exactly as a presenter's casting prose does. Last: what it was read from in the presenter's own sources row, the quiet line of facts nobody acts on, Delete, and the shots made here. Two passes in 2026-09 gave this page a two-column grid of its own, four band heads and three paragraphs explaining the mechanics; it measured the same word count as the product page and read as three times the work, because the cost is the number of times the eye has to stop, not the number of words.
