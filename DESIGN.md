...
- **Do** keep every state change paint-only: pressed controls never change geometry. Feedback is fill, opacity, border colour or inset shading, never `transform`, padding, border-width or size, so nothing moves under the cursor and a click that lands on an edge stays landed. The shared press list lives in `styles/foundations/interaction.css` alongside the focus list; a selected state follows the same rule, which is why neither chips nor settings rows bump their font-weight. If you scope a hover (`.sc-topbar .sc-icon-btn:hover`), spell it `:hover:not(:active)` — a scoped hover out-ranks the shared press fill and the control will hold its hover tone through the press, registering nothing.
