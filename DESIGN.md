...
- **One guide, everywhere, in two voices** (`guidedTasks.ts` decides, `layout/GuideHost.tsx` renders, `layout/Coachmark.tsx` draws). A **coach** holds the screen it is on: a curtain over everything but the step's live surfaces, and a card pointing at what matters. A **card** is the same card with no hold, pointing at something that just happened (a shot rendering, a shot finished, a take that failed, a new version). Everything else is **quiet**: the task stays in hand and nothing is drawn, where the surface already speaks for itself (the presenter studio's own questions, a build running). There are no hint lines and no inline notes: a word from the guide always looks the same.
...
- **The spotlight** (`layout/coachGeometry.ts`): the curtain is `rgba(0,0,0,.66)` under a static `blur(6px)`, only its opacity fades in; `.80` without blur where backdrop-filter is unsupported or transparency is reduced. The windows are the live surfaces' real shapes (`data-guide-shape`, or the surface itself), each with its own computed corner radius plus 3px of air; a shape with no corners of its own (a block of text) gets 10px of air and a 14px corner. They are cut from one mask the size of the screen, so touching windows join. A quiet rim (`rgba(255,255,255,.16)`, 1px on the curtain, never a glow) traces each window's outer edge, and a button inside a window wears a thin ring of its own. The curtain's catch panels take the page's presses; everything outside the windows is `inert` (`layout/coachLock.ts`). Steps are derived, never counted: a step is the first rule that is true from the outcome back, from facts the product publishes (`guideFacts.ts`: Create's composer, the open shot's composer, the presenter studio's open question) and what the server says the task has sent and the brand holds.
...
- **The first shot** ends at the first finished picture, not at Generate. Coach on the add control (**Every shot starts from ingredients**). The picker opening is a step, not the task: the card follows beside the picker (right, then left; above it where neither side has room, the picker giving up the height), and the picker and the composer stay live windows for search, tabs, scroll and picking (**A product is what you're shooting**). What is picked is read, not expected: a product first asks who appears or where (**Add who appears, or where**), a presenter, a scene or a colour first says what a product adds (**Add what they're showing**), all three kinds say the ingredients are in; each offers **Continue**, which closes the picker the way its own close does and hands the caret to the brief. Then the brief line (**Now direct it**) and Generate (**Make the shot**). The wait is a card on the rendering tile (**Your shot is being made**; its X only puts it away), the result a card on the finished tile (**Your first shot**, Done), a failure a card on its tile (**That one didn't finish**) that gives way the moment the brief is rebuilt. With nothing to generate with, the coach points at the composer's own setup door (**Connect image generation**). Closing the picker empty, removing every chip, words with no ingredient: all read back as add. Back reviews add, direct and generate, never a surface that opened.
- **Feature tasks begin when someone reaches for the thing, never after another task.** From First steps, or for someone new the first time they open a creation dialog, the presenter studio, or type into an open shot's composer (opening a shot is not asking to refine it). A surface begins its task at most once per visit. A task left on another surface gives way to the next; the first shot never does. Refining: a coach on the shot's composer (**Change one thing at a time**), quiet while it draws, a card on the new version (**This is a new version**, Done). The presenter studio is held at three moments only: the start (**A presenter is who appears**), the face (`identity`, `revision`: **Settle the face first**, the portrait's well live beside the question) and the save (`save`, `blind`: **Save to cast them**); every other question, on the photos road and the from-scratch road, is quiet. A product or scene dialog is held with its card beside it (**A product is what you're shooting**, **A scene is the world a shot lives in**). A product, presenter or scene task finishes when the brand holds one more than when it began; closing its surface does not end it, and a presenter left with a draft is continued, that exact draft, from First steps.
...
- **First steps** (`views/home/FirstSteps.tsx`) is a compact panel under Home's greeting, above the create grid: a hairline frame with no fill, **First steps**, a quiet count (`2 of 5`, never a percentage) and a bare X, then five actionable steps in a row (three, then two columns narrower; one sideways-scrolling row on a phone), each a state mark, a title and one line on why: **Make your first shot**, **Refine a shot**, **Add your own product**, **Cast a presenter**, **Build a scene**. A to-do is a hollow ring, the step in hand a dot and a **Continue ...** title, a done step a faded tick. Each step begins its task where it happens. A step is ticked by what the library holds and latched, so deleting the thing never takes the tick back. Someone new sees it from the start; someone upgrading only after asking. The X hides it with an Undo; Help's **First steps** brings it back, and keeps it even when everything is done. When every step is done it leaves.
- **The words** teach the product, not the chrome: a title that names the idea and one or two sentences on why, no walkthrough of a control its own label explains (press, tap, click, +), none of unique, magic, credits, compare or heatmap, and never the title of the surface they sit on. `guidedTasks.ts` holds every word, the welcome's included, and `test/guidedTasks.test.ts` enforces the rules.
