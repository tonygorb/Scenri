...
- **The welcome** (`views/WelcomeDialog.tsx`) is the one interruption: the `DialogSheet`, the first time a main page is ready, rested and idle (no dialog, nothing generating), three finished examples fading in over it, **Welcome to *Scenri***, and a choice. *Take the tour* starts the page's tour; every other way out (Skip tours, the X, Escape, the backdrop) means no tour begins on its own. Page tours never start before it has been answered.
- **A page tour** (`tours.ts` is the data, `layout/TourHost.tsx` decides, `layout/Tour.tsx` draws) belongs to one of the five main pages only, Home, Create, Products, Presenters and Scenes, and has two or three stops on the real controls. It is a spotlight, not a curtain: the page dims a step and stays usable, the light takes no clicks, focus never moves, and the card is the floating coat without a tail. A stop names its target by selectors, first visible wins, so a control that exists only at some widths falls back or is skipped. A stop that is already taken is skipped (`doneWhen`), and doing the step moves the tour on, so arriving mid-task is never taught what was just done. The composer and the libraries publish what a stop reads (`data-tour`, `data-ingredients`, `data-words`); the host never asks a screen to report. Anything else on screen, a dialog, a sheet, a menu surface, the attach panel, the lightbox, hides the tour and it comes back at the same stop. Leaving a page keeps the stop for the session.
- **Ending** a tour by *Done*, by its last step, or by skipping it means it is never offered again; sending a shot is what the Create tour teaches, from anywhere. A second skipped tour means the person does not want tours, and none starts on its own after that. Escape skips only when no field, menu or dialog is taking the key.
