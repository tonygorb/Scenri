...
A chip is an inline atom in real text flow, and the gap beside it is a real space character, never a margin or a container gap: the line keeps exactly one space on each side of a chip, `white-space: pre-wrap` renders it, and that is the 4px the frames draw. A side margin on the chip doubles it. Between two chips, and after the last one, that space is the chip's, not the sentence's, and the keyboard treats the two as one unit: one press crosses a chip, the key that faces a chip removes it (Backspace after it or its space, Delete before it or its space), and the far edge of its space is the only caret stop in the gap. `normalizeLine` is the one rule for all of it; a keystroke runs it only when `lineIsCanonical` says the line is short of it. Prose beside a chip keeps its own spaces and is stepped through a character at a time. On a fine pointer the caret in that gap is drawn at the gap's midpoint (the browser's own can only sit at an edge); on touch the platform's caret and handle are left alone, and a tap in prose keeps the platform's word-snapped caret, with the line correcting only taps in its padding. The rules live in `composer/line/keys.ts`, `composer/line/ghostCaret.ts`, `composer/line/query.ts` and `composer/line/invariants.ts`.
