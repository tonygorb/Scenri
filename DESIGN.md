...
### First use: welcome, tours and the help menu
Only an install that was new at its first boot is taught anything, and each thing is offered once (`GET /api/guide`, the install's record, so a phone on the network and a second browser agree). An upgraded library is never interrupted; everyone can ask.
- **The welcome** (`views/WelcomeDialog.tsx`) is the one interruption: the `DialogSheet`, the first time a main page is ready, rested and idle (no dialog, nothing generating), one product shot in three worlds fading in over it (`welcomeSet`), **Welcome to *Scenri***, one line (**Product shots on brand, from your products, presenters, scenes and brand kit.**) and a choice. *Take the tour* starts the page's tour; every other way out (Skip tours, the X, Escape, the backdrop) means no tour begins on its own. Page tours never start before it has been answered.
- **What the tours teach** is the studio, never the chrome. Scenri keeps a brand's ingredients (products with real photos, presenters cast once, scenes as reusable photoshoot worlds, the brand kit), a shot is a brief made from them, the chips carry the real references and the words only art-direct around them, and the win is a set: many shots, the same product, the same people, the same world. A stop's title names the job the page does; its body is one or two sentences on why the page exists. No stop walks through a control its own label already explains ("Press +", "Try one now"), and none uses unique, magic, credits, compare or heatmap. `tours.ts` holds every word, the welcome's included, and `test/tours.test.ts` enforces the rules.
- **A page tour** (`tours.ts` is the data, `layout/TourHost.tsx` decides, `layout/Tour.tsx` draws) belongs to one of the five main pages only, Home, Create, Products, Presenters and Scenes, and has two or three stops on the real surfaces. It is a window in a curtain: the page behind darkens a step further and softens under a static 4px blur, and it is inert (`layout/tourLock.ts`), so it takes no press, no Tab and no reader cursor while the stop's own surface shows through a rounded window, sharp and fully working, because it is the real control and never a picture of one. The blur is a temporary focus device, never a surface style: the card over it stays the opaque floating coat. The veil paints the window with a mask and four clear panels around it take the page's presses (a clipped window was measured still blurring across itself in Chrome). A window stops at fixed chrome floating over its target, so it shows only what can be pressed. Live regions, toasts and the update float stay live.
- **The card** is centred on its target along the side it takes, flips to the other side when there is no room and never covers the target (Floating UI, the engine Radix already positions with), and points at it with the preview card's tail. A stop with a larger surface (`region`, the whole composer) keeps the card clear of the surface while the tail still aims at the control, which wears a thin ring. The card carries only an **X** (no disc, a wide reach; *Close tour*), a quiet **Back** when a stop was already shown, a quiet `1 / 3`, and **Next** or **Done**. Back walks the tour's own history, never the product's: a chip added or a page opened stays exactly as it is, and a stop reached by Back is shown even if its step has since been taken. Doing the step still moves the tour on. Between stops the card fades out, the words change, a target below the fold is brought up once in its own pane, and the card fades in beside it; it never slides across the page. Focus goes to the card when a stop opens unless it is already on the card or the surface; Tab moves between the two only; Escape closes the tour and reaches nothing else; page shortcuts do not fire while the card or the curtain holds the key. Reduced motion removes every fade; reduced transparency drops the blur.
- **Stops** name their targets by selectors, first visible wins, so a control that exists only at some widths falls back or is skipped. Anything else that takes the screen, a dialog, a sheet, the attach panel, the lightbox, hides the tour and it comes back at the same stop, with the page let go meanwhile; a menu opened from the window floats above the curtain and closes on a press outside it. Leaving a page keeps the stop for the session; leaving from a tour's last stop finishes it.
- **Ending** a tour by *Done*, by its last step, or by closing it means it is never offered again. The X on the last stop is finishing, not skipping; anywhere else it is a skip, and a second skipped tour means the person does not want tours, so none starts on its own after that. A shot sent while the Create tour is on screen moves it on; a shot sent from anywhere else teaches nothing, so someone who first sends from Home still meets the Create tour.
- **What's New waits for someone who knows the version.** A new install's first boot counts its own version's notes as read (the same decision that makes it eligible), so the notes arrive with the next update and never describe the version being installed. While first use is still going (the welcome unanswered, a tour on screen, the Create tour not yet learned and tours not turned off), What's New never opens on its own, and a session that introduced Scenri never ends in it; when first use ends, the running version counts as read. The unread dot carries the notes meanwhile. Closing the notes by any way out, browser Back included, counts as read.
- **The refine row** is the one sentence left in the composer's notes tray: after a first finished shot on the hub, the next time the brief takes focus, **To refine a shot, open it and say what to change.**, never while a tour is on screen.
- **The help button** (`layout/HelpMenu.tsx`) is where any tour comes back, beside the help the app already had: *Tour this page*, *Start the tours over* (the welcome again; taking it forgets every tour, so each page tours on its next visit, for any install that asks; declining it changes nothing), *Keyboard shortcuts* (Create), *What's new*, *Set up image generation* (only while nothing can generate), *About Scenri*, *Scenri on GitHub*. From 1024px it floats in the bottom-right corner at the docks' level, with toasts stacked above it; it steps left of the assets rail, which from 1024px is always a column; below 1024px that corner belongs to the composer and the tab bar, so it is a top bar button beside the bell. The brand menu's menu markup, one row per item, no gold.
- **A new tour or stop** needs a hesitation the interface cannot fix with its own words first. Creation flows, detail pages and the presenter studio are guided surfaces already and carry none.
