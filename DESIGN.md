...
A scene is the place and its light around a shot, and a shot is told it in words. It is made in the presenter's studio (the same frame, transcript, composer and stage, `.sc-pstudio[data-kind="scene"]`, at `/scenes/new` and `/scenes/:sceneId/edit`, a child of the library and of the scene's page), asking a place's questions and never a person's. The first question has two doors, pictures of the place or a few questions, and a sentence typed there is the place itself. The questions are five rows, each skippable and each answerable in words: what kind of place, what light, the feeling, what it is made of, and whether it is built around someone (a position, never anyone). A row is drawn cards or words on chips, never some of each (`DRAWN` in `sceneRows.ts`), and all five carry cards. The cards follow the selection-art rule: Where is six places at one camera height; the other rows are one plinth in one plaster corner, drawn once, with only the chosen thing changed. Once the place is given it is read, the one thing the studio does on its own, and read back as **What your shots are told**, the words every shot will get, with one **Draw the scene** under it and the line open for anything to add or leave out; nothing is drawn without that press. The picture lands with one decision, **Use this scene**, **Try again** (the same words, a new picture) or **Change something** (a sentence that revises only what it names and changes the picture from the one before), and every version is a pair of words and picture that **Put back** restores whole. The name is asked while the first picture draws, with the reader's suggestion as a starter, and a sentence that names it ("call it Stone Hall") names it wherever it is said and draws nothing. The conversation reads forward here too: an answer the picture was drawn from asks once before its pencil opens it, because changing it takes back the pictures with everything else asked after it, and the Confirm says the change line is the way to keep the picture and alter one thing. A version is the candidate until Use; nothing is saved before it and nothing rejected reaches Create. A sentence asking to cast a presenter or place a product is answered in one line (they belong to Create), and a scene's pictures are read into words, never sent with a shot, except a figure-led scene's preview, which is the plate a shot beside a presenter conditions on. A scene's own page is the one record that leaves the centred `sc-lookpage` shell Product and Presenter share, because a place is judged beside its picture and not under it: two columns inside 1240, the picture on the left at the size it was drawn (its own frame, bounded by the screen, never cropped into a card) with what has been **Made here** under it, and a 420 reading column on the right holding the name, its filing, the lede and the verbs, then **Ways to shoot it** and **What your shots are told** (Light, Camera, Built around, each showing its first line, with the whole of them and the long set prose behind one press), then the filing and Delete. The pictures it was read from sit with the pictures, under the place, as a quiet labelled row. A record page explains its mechanics once, in a tooltip or an empty state, never in a paragraph under every band: three of those turned this page into prose nobody reads twice. One rhythm of 40 between every band. On a phone the same three blocks stack in the order they are read: the name, the place, the record. Nothing there draws or changes a picture: the words are read-only, name and filing live in Details, and **Edit scene** opens the studio at the record, spending nothing.
