...
- **The first shot is built with them, and made on us.** Nothing is ever put in the brief unasked: Scenri keeps a recipe it ships in mind (`starterRecipe`) and offers it a part at a time, where that part is added. What you are shooting (**Start with what you are shooting**), who shows it (**Now who shows it**), where it happens (**And where it happens**), then the words (**Say how to shoot it**). Each step has two answers that are equally good: pick their own, or take ours with one press (**Use ours**, **Write one for me**), and the card lists the three ingredients, ticking each as it lands. The picker opens on the kind being asked for and moves on with the step, so a step about presenters shows presenters; while it is open it is the step, with the picker and its own toggle live and **Next** to close it. The words come with the shape, the number and the size that recipe was shot with (**Set to suit this shot**), leaving one thing to press (**Make the shot**). Pressing it asks no engine while the brief is still the one Scenri had in mind, ingredients and words alike (`showcaseId` on `POST /api/nodes`, before the engine gate, `engineId: 'local'`, nothing charged): the picture that recipe already made lands as a real shot of theirs, so the first result is immediate and good, and the card says it is on us beforehand rather than after. A part of their own makes it their shot, generated the real way, and that, at the last step, is the only time anyone is sent to set up image generation (**Connect image generation**). Taking a part back out asks for it again. The wait is a card on the rendering tile (**Your shots are on the way**; its X only puts it away), the result a card on the finished tile (**Your first shot**, Done; on a tile that fills the screen the card comes inside its bottom edge), a failure a card on its tile (**That one didn't work**) that gives way the moment the brief is built again. Back reviews the parts already answered, never a surface that opened.
