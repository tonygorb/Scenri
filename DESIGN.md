...
A chip is an inline atom in real text flow, followed by a space the way a mention is in any text field. That space is the chip's: it is the 4px gap the frames draw between two chips (`white-space: pre-wrap` renders it; the chip carries no side margin, which would double it), and it is where the caret lives, which a phone needs, since a touch caret only exists in text. Between two chips there is exactly one such space. Everything else in the line is the user's: the line never puts a space beside their words and never takes one away, so text typed flush against a chip stays flush until they type the space themselves. A chip and its space are one unit to the keyboard: one press crosses both, the key that faces a chip removes it (from keydown for a hardware key, from `beforeinput` for a phone's keyboard), and a press at a line edge with nothing on its side is swallowed, because both engines otherwise clear the line. On a fine pointer the caret in the gap between two chips is drawn at the gap's midpoint (the browser's own can only sit at an edge); on touch the platform's caret and word-snap stand, and a tap is corrected only in the line's padding. The rules live in `composer/line/invariants.ts` and `composer/line/keys.ts`; the drawn caret in `composer/line/ghostCaret.ts`.
