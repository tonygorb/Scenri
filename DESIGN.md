...
A person is established visually, at a place of its own, not in a dialog. A modal is for a short task you finish and return from; a person takes minutes, five drawn views, a refine loop and a draft that outlives the session, and the dialog it wore promised quick-then-back over a workspace. So the studio has an address, `/presenters/new` for a fresh start and `/presenters/new/:draftId` once there is a draft (`PresenterStudioRoute`, `.sc-pstudio`), a child route of the library the way the shot overlay is of the hub: full-bleed over it at the same z (80), the library still mounted and scrolled where it was, so leaving is instant, Back and the close are the same move (every move inside replaces), a reload lands on the same step, and a draft can be handed to someone as a link. The old `?new=presenter` forwards there; Product and Scene, which are short forms, stay dialogs. Laid out as the Figma studio frames draw it: the stage on the left padded 40 with the round close at 40/40, a 500 rail on the right; on a phone the same pieces stack into one column, the head clearing the notch, with one scroller and a bottom that stays put. Setup is the frames' form: the two ways to start as a words-only segmented control in the title row (From scratch / From photos at the head's own 30, sized by its words, arrows move between them; on a phone, where the head's close takes that row, it drops to a two-up line under the title), then the three things a roll cannot guess and the sentence that carries the rest: Who they are and Age as rows of chips (Woman / Man / Androgynous, then the five decades: the same kind of fact, a few ordered words, so the same control; the drawn busts this row once had read as a casting, pick this person, when the row is a steer, describe them as a woman, and the word says only that), Skin as six swatches, Hair as six and any colour at all through the app's own picker (named for the prompt on the way out, since a hex is dropped or guessed), both colour rows light to dark, each swatch the colour as a solid 28 disc with a hairline on it (the chips' height), twelve apart, and Describe the presenter in the composer or the four photo places Front / Left / Back / Right; the name is offered, never required. The sentence itself is not in the body: it is the composer at the bottom of the rail, the same card that later corrects the person, so the place a person types never moves. Each of the three trait rows puts words in front of the sentence in the order a person would say them (`castSentence`), and each drops out when the sentence already covers it rather than saying it twice. The rail keeps one rhythm, twenty between blocks, thirty-two at a group boundary (after the name, and before the composer), twelve from a caption to its control, captions at 11.5 / 500 so they sit above the 15 inputs rather than beside them, every input on one ground at 44 tall including the resume row, and one selection language, monochrome: the app's fill on a chip, an ink ring outside a swatch with the panel showing between so the colour is never covered, never a coloured badge on a choice. No caption echoes its value: a chosen card, chip or disc is its own answer. Whatever is only a mark says its word in the app's tip on hover and focus (`Tip`, never a native title): each swatch its colour's name, the hair plus Any hair colour, the stage close Close (esc), a photo's remove Remove, and the dimmed pill why it will not go (the reason the line under the card gives, or Nothing typed yet). The two ways to start are a segmented control, not a tab strip: this is a choice of what the person is made from, and it changes the form under it. Before anything is drawn the stage carries the five frames the person will arrive as, labelled and empty, rather than one grey icon in a very large box. When neither the steer nor the sentence says who this is, one line under the description says so and what it costs (`whoHint`). Categories are not asked here: the engine reads them off the photographs or off the portrait it drew, and a form cannot ask what a person is cast for before that person exists. They appear in review as Filed under, one line already carrying the engine's reading, correctable through the app's own menu. Then one face, drawn and decided: Use this person, Try again, or a sentence that adjusts it. Then the front, left, back and right views, each drawn from the views used before it, decided in turn with Use or Try again, the next one starting on its own. The strip under the stage is the progress and the navigation: five 90 x 112 tiles at a 98 pitch with the label centred under each (Avatar, Front, Left, Back, Right), the one on the stage outlined in ink and the rest at half strength, a mark on what stands, no step numbers. The rail reads as a transcript, You and Scenri, with no model ever asked for words: Scenri's lines are the build's own status, with copy and draw-again beside them and the decision buttons under them. The composer at the bottom is an editing field: who this is sits on it with Change (the name, notes and categories behind it); a chip in the card names the picture Refine will redraw and follows the sentence as it is typed, and the line under the card says what that means; a sentence changes the person (the face is redrawn and the other views follow once it is used) or the view on the stage; a revision offers Use or Keep previous; a photograph is never redrawn. The title reads Create presenter until the five views stand, Refine presenter after. The name comes last, with Details behind one quiet button. Leaving keeps the draft, and coming back to `/presenters/new` resumes the one this session pointed at, its address taking over; Start over is the quiet `Confirm`. No published state, no assistant prose, no engine call spent on words. Without an engine, From scratch shows the setup card and From photos saves the photographs as they are.
