...
- **Split:** one primary control with two hit regions, for a verb that has a usual thing and a short list of neighbours (the top bar's New). The filled half performs the usual thing, and leads with a 24px gold disc carrying the plus, inset 4px so the disc and the round end share a centre, then the word at 13.5/500; a 1px divider in the fill's own ink at 22%; the caret half opens the menu. Its shape is the primary's full round, so each end is a half-circle at any height, and each half carries that curve on its own end so a focus ring follows the shape. Its press is the one sanctioned exception to the primary's opacity step: each half shades its own fill (10% hover, 18% pressed, of `--sc-inv-fg` over `--sc-inv-bg`), because an opacity step cannot say which half a press will land on and saying that is the control's whole purpose. Never for a verb with no usual thing: that is a menu wearing a loud coat.
