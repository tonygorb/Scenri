...
Scenri is learned by using it. First use never tours the interface; it stays with someone while they do one real thing to a real result, reading what the product holds rather than counting clicks. The record is the install's (`GET /api/guide`, `POST /api/guide` intents only), so a phone on the network and a second browser agree. Who is new is decided once, at the first boot of a build that has the record: a home with no brand is new; a home with brands is someone upgrading and is never interrupted. Everyone can ask for First steps from Help.
- **Three voices, one per step** (`guidedTasks.ts` decides, `layout/GuideHost.tsx` renders, `layout/Coachmark.tsx` draws). A **coach** holds the page: a window in a curtain, the page behind darkened, softened under a static 4px blur and inert (`layout/coachLock.ts`, so no press, no Tab, no reader cursor), while the step's own surfaces show through sharp and fully working, because they are the real controls. The mask paints the window and four clear catch panels take the page's presses; a window stops at fixed chrome floating over it. A **card** is the same tailed card with none of the hold: it points at something that just happened and focus stays put. A **note** is one sentence in a slot the surface already has (`layout/GuideSlot.tsx`, the hint banner's quiet voice): the picker's foot, the open shot's tray, a creation dialog's form, the presenter studio above its composer. Everything else is **quiet**: the task stays in hand and nothing is drawn. Only the first shot uses the coach; later tasks speak in notes.
- **Steps are derived, never counted.** A step is the first rule that is true, checked from the outcome back: a finished picture, a send in flight, a take rendering, a failure, no engine, then the brief itself. A send empties the brief, so reading forward would put someone back at the start the moment they succeeded. The facts come from the product, never from guessing at the DOM: Create's composer publishes where its brief stands (`guideFacts.ts`, only when it changes), the presenter studio publishes the question it is asking, and the server returns what the task has sent since it began (`activeNodes`, by brand and the database clock) and what the brand holds (`counts`). Leaving, coming back, Back, Forward and a reload all land on the step that is true.
- **The welcome** (`views/WelcomeDialog.tsx`) is the one interruption, once, for someone new, the first time Home or Create is ready, rested and idle (no dialog; nothing generating or building; a store import running in the background does not count): one product shot in three worlds (`welcomeSet`), **Welcome to *Scenri***, **Product shots on brand, from your products, presenters, scenes and brand kit.**, a line saying what the first shot will ask (a short setup when nothing can generate, otherwise a few minutes with a product of theirs or of ours), and **Make your first shot** or **Not now**. Every way out that is not taking it is Not now, and First steps keeps the offer.
- **The first shot** is the activation, and it ends at the first finished picture, not at Generate. On Create: a coach on the add control (**Add what you're shooting** / Every shot starts from real references. Scenri's own products are ready now.); the picker opening moves the words into its foot with the composer and the picker left open (Choose a product. Its real photos keep it exact in every shot.; once one is in, Add a presenter or a scene if you like, then close the panel to direct the shot.); a coach on Generate (**Direct it, then Generate** / A few words for the place, the light and the moment. The product already carries its look.). The send lets go of the page at once and the wait is quiet (the tiles have their own clock; a reader hears Your first shot is on its way.). A finished picture ends it on its own tile, as a card (**Your first shot** / Open it and say what to change. The rest of the shot stays as it is., Done); opening that shot ends it too. A failed take is a card on its tile (That one didn't finish. The tile says why, and trying again keeps your product and words.) and never a finished shot; building the brief again brings the coach back. With nothing to generate with, the first step is a coach on the composer's own setup door (**Connect image generation** / Your first shot needs it. The rest of Scenri works meanwhile.) and the setup dialog it opens is the composer's, never a second one. The picker's close and a press on the curtain never skip a step: closing it with nothing picked goes back to adding.
- **The coach card** is centred on its target, flips when there is no room, never covers it (Floating UI), and points at it with the preview card's tail; a target inside a larger live surface wears a thin ring. It carries a bare **X** (*Close guide*, a wide reach, no disc, no Skip), a quiet **Back** when a card was shown before it in this task, and **Done** only where the task is finished. Back shows the card before for review and touches nothing in the product; **Next** returns. Escape puts the coach away until the step changes, unless a live surface answered it first (the picker closing). Focus goes to a coach card when it opens unless it is already in the card or its surfaces; a card and a note never take it. Between steps the card fades, the words change, and it fades in beside the new target; reduced motion removes every fade, reduced transparency drops the blur.
- **The X ends the task in hand, never the work.** The brief, the picker, the dialog and every shot stay exactly as they are; a toast says First steps on Home picks it up again, and launching it there begins it again. On a finished task the X is simply closing it.
- **First steps** (`views/home/FirstSteps.tsx`) is one quiet line on Home's greeting row, beside the greeting where there is room, under it where there is not, and one sideways-scrolling row on a phone: *First steps*, then **Make a shot**, **Refine a shot**, **Add your product**, **Cast a presenter**, **Build a scene**, and a bare X. A to-do is a hollow ring, the task in hand a dot, a done step a faded tick. No counts, no percentage, no accent, no order imposed. Each item begins its task where it happens: Create; the newest shot's overlay (the first shot when there is none); the product or scene dialog; the presenter studio, continuing the draft that task started. A step is ticked by what the library holds (a finished shot, a finished refinement, a product, a presenter of its own, a scene) and latched the first time it is seen, so deleting the thing never takes the tick back. Someone new sees it once the welcome is answered; someone upgrading sees it only after asking. The X hides it with an Undo; Help's **First steps** brings it back. When every step is done it leaves.
- **Later tasks are light.** They begin from First steps, or, for someone new, the first time they open that surface themselves (a finished shot's overlay, the product or scene dialog, the presenter studio), only if that step is not done, not closed and nothing else is in hand. Tasks never chain on their own. Refine: One change at a time works best, like warmer light or a closer crop. in the open shot's tray, quiet while it draws, This is a new version. The original is one step back. when it lands, finished when the shot closes. Product: Photos of the real product keep it exact in every shot, or import your store below. Scene: Build a place once and shoot in it again. Photos of a real place work best; a line of direction works too. Both finish when the brand holds one more than when the task began, and leaving the dialog with nothing made and nothing building ends the task quietly. Presenter: Cast them once, and the same person fronts every shot. at the first question, Every shot will use this face, so settle it here. at the face decision, quiet through every other question; it finishes when the presenter is saved, and a studio closed with a draft keeps the task, so First steps continues that draft.
- **The words** teach the product, not the chrome: one or two sentences, no walkthrough of a control its own label explains (press, tap, click, +), none of unique, magic, credits, compare or heatmap, and a note never repeats the title of the surface it sits in. `guidedTasks.ts` holds every word, the welcome's included, and `test/guidedTasks.test.ts` enforces the rules.
- **What's New waits for someone who knows the version.** A new install's first boot counts its own version's notes as read. While someone new has not answered the welcome, while anything of the guide is on screen, and while the first shot is in hand, What's New never opens on its own, and a session that introduced Scenri never ends in it; when the welcome is answered the running version counts as read. The unread dot carries the notes meanwhile.
- **The help button** (`layout/HelpMenu.tsx`): *First steps*, *Keyboard shortcuts* (Create), *What's new*, *Set up image generation* (only while nothing can generate), *About Scenri*, *Scenri on GitHub*. From 1024px it floats in the bottom-right corner at the docks' level, with toasts stacked above it, stepping left of the assets rail; below 1024px it is a top bar button beside the bell. The brand menu's menu markup, one row per item, no gold.
