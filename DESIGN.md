...
- **Keeping is the composer's to say, once.** A refinement borrows the identity of the shot it refines (product, person, mark, reference) so the engine keeps them; the band over the refine field lists them under one word, `Keeping`, and each chip's x switches it off: the chip stays in the band, muted, so one more press switches it back on before the send, and while off it is left out of the request by key (`drop`) and out of the record. The refinement made without it does not list it at all, because it was never carried into that version. Said forward, never as "carried over": that read as a fact about this shot, and on the Original there was nothing carried at all. The record under BRIEF is the ask alone; the same list there under another meaning was two lists with two meanings on one screen. The world is never a chip: it is kept through the photograph and is the Original's to say.
- **The picture answers a right click.** A right click or a long press on it opens the shot's own verbs, the ones the header carries, the way a tile in the feed does; the browser's menu never shows over a shot. The verbs are Download (one click, the PNG itself), Keep, Copy, Archive and, once archived, Delete. There is no Compare: the trail puts the source one click away and the stage cross-fades between the two, which is the before and after.
