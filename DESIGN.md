...
### First use: guided tasks and First steps
Scenri is learned by using it. First use never tours the interface: one guide stays with someone while they do a real thing to a real result, reading what the product holds rather than counting clicks. The record is the install's (`GET /api/guide`, `POST /api/guide` intents only), so a phone on the network and a second browser agree. Who is new is decided once, at the first boot of a build that has the record: a home with no brand is new; a home with brands is someone upgrading and is never interrupted. Everyone can ask for First steps from Help.
- **One guide, everywhere, in two voices** (`guidedTasks.ts` decides, `layout/GuideHost.tsx` renders, `layout/Coachmark.tsx` draws). A **coach** holds the screen it is on: a curtain over everything but what the step asks for, and a card pointing at it. A **card** is the same card with no hold, pointing at something that just happened (a shot rendering, a shot finished, a take that failed, a new version). Everything else is **quiet**: the task stays in hand and nothing is drawn, where nothing is being asked (a picture drawing, a build running, the look questions after the first). There are no hint lines and no inline notes: a word from the guide always looks the same.
- **The coach follows the screen that owns the moment.** On the page it is drawn in the body. Over the presenter studio, a creation dialog or the open shot, which trap focus and sit above the page, it is drawn inside that shell (`container`), so its card is reachable and its lock holds only the shell. It never draws a card that cannot be pressed.
- **The spotlight** (`layout/coachGeometry.ts`): the curtain is `rgba(0,0,0,.66)` under a static `blur(6px)`, only its opacity fades in; `.80` without blur where backdrop-filter is unsupported or transparency is reduced. **The stage stays whole; the control is pointed at inside it.** A window is the surface a step happens on (the composer, the picker, the studio's question, the dialog), cut at its own edge with its own corners and no halo of air (a shape with no corners of its own gets 10px and a 14px corner), from one mask the size of the screen so touching windows join, traced by a quiet rim (`rgba(255,255,255,.16)`, 1px, never a glow). Inside the stage, whatever the step does not ask for recedes to a third of its strength and cannot be used (`[data-guide-stage] [data-sc-coach-inert]`); the one control it asks for stays at full strength, wears a 2px ring 4px clear of its edge that ripples twice and rests, and the card's pointer lands on it. Never a window cut around a lone control: a button floating in a hole reads as a broken page. What can be used (`live`) is the control, plus the popover, sheet or picker it opens, which join the windows while open. Everything else is `inert` (`layout/coachLock.ts`) and the curtain's catch panels take its presses. A control's own popover or the phone's settings sheet is part of the step, never a dialog that pauses the guide. Steps are derived, never counted: a step is the first rule that is true from the outcome back, from facts the product publishes (`guideFacts.ts`: Create's composer, the open shot's composer, the presenter studio's open question) and what the server says the task has sent and the brand holds.
- **The welcome** (`views/WelcomeDialog.tsx`) is the one interruption, once, for someone new, the first time Home or Create is ready, rested and idle, held behind the guide's own curtain (`DialogSheet tone="guide"`): one product shot in three worlds, **Welcome to *Scenri***, **Product shots on brand, from your products, presenters, scenes and brand kit.**, what the first shot will ask of this install, and **Make your first shot** or **Not now**.
- **The first shot** ends at the first finished picture, not at Generate, and walks every control once, each a step that is done by doing it. It asks for **one product, one presenter and one scene**, as a checklist on the card that ticks each as it lands: at the add control (**Add your ingredients**, or **Now add a presenter and a scene** when some are in), then in the picker, with the card beside it (right, then left; above it where neither side has room, the picker giving up the height) and only the picker and its own toggle live (**Add a product, a presenter and a scene**, **Add a presenter and a scene**, **Add a scene**; the words say what the next one missing gives). Any order is fine; each pick moves the picker to the tab of the next one missing, and an open row takes it there. **Continue** is on the card from the start and waits for all three (**That's everything a shot needs**), then closes the picker the way its own close does. An ingredient the library has none of is never asked for; a colour or an image is welcome but is none of the three. The brief line (**Describe the shot**), finished by **Continue** or Enter, never by the first word typed; a send before the Generate step is read as that Continue. Then the shape (**Choose the shape**), the number (**Choose how many**) and the size (**Choose the size**), each opened and answered, keeping the current value counting as an answer; where the composer is narrow all three sit behind its one settings control (**Choose shape, number and size**). Then Generate (**Make it**). The wait is a card on the rendering tile (**Your shots are on the way**; its X only puts it away), the result a card on the finished tile (**Your first shot is ready**, Done), a failure a card on its tile (**That one didn't work**) that gives way the moment the brief is rebuilt. With nothing to generate with, the coach points at the composer's own setup door (**Connect image generation**). Closing the picker short, removing a chip, words with no ingredient: all read back as add. Back reviews the controls already answered, never a surface that opened.
- **Feature tasks begin when someone reaches for the thing, never after another task.** From First steps, or for someone new the first time they open a creation dialog, the presenter studio, or press or type into an open shot's composer (opening a shot is not asking to refine it). A surface begins its task at most once per visit. A task left on another surface gives way to the next; the first shot never does. Refining: a coach on the shot's composer (**Change one thing**), quiet while it draws, a card on the new version (**A new version**, Done). **The presenter conversation is guided at every answer that shapes the person**: each card sits beside the conversation, frames the open question (or the question and the composer, where the answer is typed) and leaves only its answers live. The start (**Create a presenter**), the first look question (**Build their look**), a description (**Describe them**), photos (**Start from real photos**), distinctive details (**Anything distinctive?**), drawing (**Draw the presenter**), the face with the portrait in view (**Decide the face**), the full body (**Check the full body**), extra views (**More angles are optional**), the name (**Name them**) and the save (**Save your presenter**). The look questions after the first, the waits while it draws, and the studio's own setup and retry answers stay quiet. A product or scene dialog is held with its card beside it (**Add your product**, **Build a scene**). A product, presenter or scene task finishes when the brand holds one more than when it began; closing its surface does not end it, and a presenter left with a draft is continued, that exact draft, from First steps.
- **The X ends the task in hand, never the work**: the brief, the picker, the dialog, the studio and every shot stay as they are, a toast says First steps picks it up again. On a finished card the X is simply closing it.
- **First steps** (`views/home/FirstSteps.tsx`) is one panel under Home's greeting, above the create grid, built around **the next step**: the task in hand, else the first not done, with a picture of it, a quiet `First steps · 2 of 5 done`, its title, one sentence on what it gives, and **Start** (or **Continue** for the task in hand). Under it every step sits in a quiet row (**First shot**, **Refine**, **Product**, **Presenter**, **Scene**), each opening its own task in any order, the next one outlined. A to-do is a hollow ring, the step in hand a dot, a done step a thin tick; never a percentage, never an accent colour. A step is ticked by what the library holds and latched, so deleting the thing never takes the tick back. Someone new sees it from the start; someone upgrading only after asking. The X hides it with an Undo; Help's **First steps** brings it back, and keeps it even when everything is done. When every step is done it leaves. On a phone the next step stacks over a full-width button and the row scrolls sideways.
- **The words** are plain: a title that says what to do in a few words, then one or two sentences on why it matters and what to do, in the language of the product (ingredients, shot, presenter, scene), never of the chrome (press, tap, click, +). None of unique, magic, credits, compare or heatmap, and never the title of the surface they sit on. `guidedTasks.ts` holds every word, the welcome's included, and `test/guidedTasks.test.ts` enforces the rules.
- **What's New waits for someone who knows the version.** A new install's first boot counts its own version's notes as read. While someone new has not answered the welcome, while anything of the guide is on screen, and while the first shot is in hand, What's New never opens on its own; when the welcome is answered the running version counts as read.
- **The help button** (`layout/HelpMenu.tsx`): *First steps*, *Keyboard shortcuts* (Create), *What's new*, *Set up image generation* (only while nothing can generate), *About Scenri*, *Scenri on GitHub*.
