...
- **Carried over is what rode, and you decide what rides.** A refinement borrows the identity of the shot it refines (product, person, mark, reference) so the engine keeps them; the band over the refine field lists them under one line, `Carried over`, and each chip can be left out with its x, which keeps it out of the request by key (`drop`). The record under BRIEF then shows, under the same line, only what rode along, from the node's own record, never what the ask already names and never the world: the world is kept through the photograph and is the Original's to say. That is how "remove the person" stops listing the person, and stops sending them as a reference.
