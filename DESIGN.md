...
- **The rail is furniture only where there is room:** from 1280px, the width the assets drawer needs to dock, and never below two shots. Below that width the arrows carry the walk, and a phone swipes the picture. The rail reads with depth: a tile swells toward the middle of the column, past its resting size, and sits back toward the ends until it is gone at the very top and bottom, and the gaps between tiles shrink with the tiles, so the spacing reads as one perspective rather than as tiles rattling in fixed tracks. That is a fisheye laid in transforms each frame (`ShotRail`'s `swell`), not a view timeline: a timeline can scale a tile in place but cannot move it, and even spacing is positions moving with sizes. The column's own tracks never change, so the scroll distance never changes under the pointer. The ringed tile is centred on every step, so the shot on screen is always the large one and the column reads outward from it.
