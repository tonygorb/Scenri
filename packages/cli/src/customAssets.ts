...
/** Every image a custom asset points at, for the export rewriter. */
export function customAssetHashes(brandJson: any): string[] {
  const out: string[] = [];
  for (const c of brandCharacters(brandJson)) {
    for (const r of c?.sourceRefs ?? []) {
      const h = hashOf(r?.file);
      if (h) out.push(h);
    }
  }
  for (const s of brandScenes(brandJson)) {
    for (const r of s?.refs ?? []) {
      const h = hashOf(r?.file);
      if (h) out.push(h);
    }
    const p = hashOf(s?.preview);
    if (p) out.push(p);
  }
  return out;
}
