...
describe('startLines', () => {
  it('the studio on this computer, and where a phone finds it, on the real port', () => {
    const text = startLines({ ...base, phone: { address: 'http://192.168.1.42:4750', code: '482913' } }).join('\n');
    expect(text).toContain('Scenri Studio → http://127.0.0.1:4750');
    expect(text).toContain('on your phone → http://192.168.1.42:4750  code 482 913');
    expect(text).toContain('data dir      → /Users/me/.scenri');
    expect(text).not.toContain('4747');
  });

  it('no phone line when no phone could open it', () => {
    const text = startLines({ ...base, phone: { address: null, code: '482913' } }).join('\n');
  ...
  it('lines the arrows up', () => {
    const lines = startLines({ ...base, phone: { address: 'http://10.0.0.2:4750', code: '482913' } });
  ...
  it('every line obeys the copy rules: no long dashes, no exclamation, no emoji', () => {
    const all = [
      ...startLines({ ...base, phone: { address: 'http://192.168.1.42:4750', code: '482913' } }),
...
describe('localUrl', () => {
  it('the loopback for the default, a wildcard, or localhost', () => {
    for (const host of ['127.0.0.1', '0.0.0.0', '::', 'localhost']) {
      expect(localUrl(host, 4747, '482913')).toBe('http://127.0.0.1:4747');
    }
    expect(localUrl('::1', 4747, '482913')).toBe('http://[::1]:4747');
  });

  // bound to one named address, this computer is a device like any other
  it('one named address carries the code', () => {
    expect(localUrl('192.168.1.20', 4747, '482913')).toBe('http://192.168.1.20:4747/?t=482913');
  });
});
