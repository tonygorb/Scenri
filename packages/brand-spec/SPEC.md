...
6. **A scene is a place, not a picture.** `scenes[].prompt` describes the reusable world (environment, materials, light, atmosphere) and never names the product, brand or person staged in it (the schema rejects a `{product_name}` placeholder outright). `scenes[].refs` are the human's inspiration images. They are read into words when the scene is built, and they are the references for the scene's own preview card. An environment scene reaches a shot as text only: its `prompt`, and after it its `lighting` when that says something the prompt does not (a tool that shows a scene's light as its own line must send it too, or say it does not). A scene built around a figure also attaches its drawn preview to the shot when a presenter is attached, marked as scene context, because a figure treatment - a face covered in stickers, a body behind fabric - does not survive being written down; what is taken from it is the environment and the treatment, never the person, product or mark that happens to be in it. With nobody attached it reaches the shot as text and the figure's position stays empty: no presenter, no person. `scenes[].figure` may name the figure a world is built around when the concept depends on one - a role rather than a person - and `scenes[].figureTreatment` what has been applied to them, so a brief can hand that role to an attached presenter without the reference person's identity travelling with it. A scene owns the role and the treatment; a presenter owns the face underneath, and nothing a shot does to a figure is ever written back to a presenter record.
7. **Identity evidence outranks anything generated from it.** `characters[].sourceRefs` holds the photos a person was built from; `characters[].shots` may hold normalized views derived from those. A tool may regenerate `shots`; it must never overwrite `sourceRefs`, and must not present a generated view as more authoritative than the evidence. An edit that changes a person's pictures or identity prose is written as a new record with a fresh id and `revisionOf` naming the one it replaced, which stays in the document marked `supersededBy` so a shot made against it keeps refining against the pictures it was made from; `identityEdits` holds the accepted identity-wide instructions that record was drawn under, and a tool lists only the records without `supersededBy`.
8. **A person's origin is recorded, never inferred.** `characters[].source` says whether a person was made from a description (`synthetic`) or from photographs of a real person (`photos`); `characters[].likeness` holds the confirmation the person casting them gave for a real person's commercial likeness. A tool must keep both across edits, must not set `likeness` on a synthetic person, and must not invent a source it has no policy for.
9. **`promptName` is frozen, `name` is free.** Where both exist, generators read `promptName` and humans read `name`. Without it a record is named to the generator by `name`, which makes renaming a generation change, so tools that let people rename should write a `promptName` at creation.

### Changelog

- **0.1 draft, 2026-09:** added `revisionOf` / `supersededBy` / `identityEdits` to `characters[]`, so an edit that changes a person's pictures is a new record and older shots keep their references. Additive within the draft, no `specVersion` bump.
- **0.1 draft, 2026-09:** added `source` / `likeness` / `facial` / `skin` / `build` to `characters[]`, for people cast in Scenri's presenter studio. Additive within the draft, no `specVersion` bump.
