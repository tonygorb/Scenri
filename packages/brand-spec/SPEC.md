...
9. **A product's photographs are facts; a drawn view is an estimate.** `products[].shots[].source` is `derived` on a view a tool drew from the photographs and absent (a photograph) otherwise. A generator reads photographs first and takes colour, finish, material and every mark from them alone; a drawn view supplies shape and proportion and never outranks a photograph. `products[].cover` is display media, never a reference of higher authority. The identity sheet (`promptName`, `description`, `materials`, `primaryColors`, `preservationNotes`, `negativeConstraints`, `colorways`) says what the object is, never how a shot should be lit or staged.
