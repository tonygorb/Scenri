...
6. **A scene is a place, not a picture.** `scenes[].prompt` describes the reusable world (environment, materials, light, atmosphere) and never names the product, brand or person staged in it (the schema rejects a `{product_name}` placeholder outright). `scenes[].refs` are the human's inspiration images. They are read into words when the scene is built, and they are the references for the scene's own preview card. An environment scene reaches a shot as text only. A scene built around a figure also attaches one reference to the shot, marked as scene context, because a figure treatment - a face covered in stickers, a body behind fabric - does not survive being written down; what is taken from it is the environment and the treatment, never the person, product or mark that happens to be in it. `scenes[].figure` may name the figure a world is built around when the concept depends on one - a role rather than a person - and `scenes[].figureTreatment` what has been applied to them, so a brief can hand that role to an attached presenter without the reference person's identity travelling with it. A scene owns the role and the treatment; a presenter owns the face underneath, and nothing a shot does to a figure is ever written back to a presenter record.
