...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    fix: 'Pick another engine in the composer.',
    remedy: { label: 'Choose engine', opens: 'engines' },
    retryable: false,
  },
  ...
  },
  {
    ...
    fix: 'Open setup and check again. If Codex was just installed, restart Scenri.',
    remedy: { label: 'Check Codex', opens: 'setup' },
    retryable: true,
  },
  {
    re: /codex cli .* is too old|Scenri needs \d+\.\d+\.\d+ or newer/i,
    kind: 'setup',
    title: () => 'Codex CLI needs an update.',
    fix: 'Update Codex, then run this again.',
    remedy: { label: 'Update Codex', opens: 'setup' },
    retryable: false,
  },
  {
    re: /\brun aborted\b/i,
