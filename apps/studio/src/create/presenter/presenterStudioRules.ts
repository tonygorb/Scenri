...
export function coverageLine(d: DraftLike, canGenerate: boolean): { text: string; tone?: 'warn' } | null {
  ...
  // Nothing readable in any of them: the question standing under this says so
  // and asks what to do about it, so promising here that the views are drawn
  // from these photographs would be the studio contradicting itself in
  // consecutive lines.
  const filed = d.analysis?.photos ?? [];
  if (filed.length && filed.every((p) => p.usable === false)) return null;
