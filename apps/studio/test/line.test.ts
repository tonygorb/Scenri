...
describe('normalizeLine invariants', () => {
  it('a structural edit shapes exactly one space on each side of a chip', () => {
    renderLine(
      root,
      [
        { t: 'text', v: 'a   ' },
...
describe('the typing path: lineIsCanonical, then normalizeLine', () => {
  ...
  it("spaces beside prose are the user's: nothing is taken from them, nothing put beside them", () => {
    ...
    normalizeLine(root, { shapeProse: false });
  ...
  it('prose typed flush against a chip stays flush until the user types the space', () => {
    ...
    // the next structural edit reads the sentence and shapes it
    normalizeLine(root);
    expect(shape()).toEqual(['"se fdsfdfsdf "', '<product>', '" on"']);
  });

  it('a Backspace before a chip that starts the line is swallowed', () => {
    renderLine(
      root,
      [
        { t: 'product', id: 'p1' },
    ...
    caret(0); // the empty host before the first chip
    expect(deletionAtLineEdge(root, 'Backspace')).toBe(true);
    expect(deletionAtLineEdge(root, 'Delete')).toBe(false);
    caret(2); // the space between them
    expect(deletionAtLineEdge(root, 'Backspace')).toBe(false);
    caret(4); // past the last chip's space: a Delete there is the browser's, and takes nothing
    expect(deletionAtLineEdge(root, 'Delete')).toBe(false);
    renderLine(
      root,
      [
