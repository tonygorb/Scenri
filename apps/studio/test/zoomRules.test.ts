...
describe('limits and the look', () => {
  it('looks at actual size when that is bigger than fit, else at twice fit', () => {
    const l = limits(FIT);
    expect(l.look).toBe(1);
    expect(l.min).toBe(FIT);
    expect(l.max).toBe(2);
    // a picture already shown at its own size looks at twice that
    const same = limits(1);
    expect(same.look).toBe(2);
    expect(same.max).toBe(4);
    // and one shown nearly at its own size does not bother with a 4% step
    expect(limits(0.97).look).toBeCloseTo(1.94);
  });

  it('a click goes to the look and a click goes back', () => {
    const l = limits(FIT);
    expect(toggleTarget(FIT, l)).toBe(1);
    expect(toggleTarget(1, l)).toBe(FIT);
    expect(toggleTarget(0.7, l)).toBe(FIT);
    expect(isFit(FIT * 1.001, l)).toBe(true);
    expect(isFit(FIT * 1.1, l)).toBe(false);
  });
});
...
describe('zoom and pan', () => {
  ...
  it('keeps the picture point under the click where it is, past fit as well as inside it', () => {
    const v = centred(FIT, vp, nat);
  ...
  it('clamps the scale to the limits and the pan to the edges', () => {
    const v = centred(FIT, vp, nat);
    expect(zoomTo(v, 100, 500, 400, vp, nat, l).scale).toBe(2);
    expect(zoomTo(v, 0.0001, 500, 400, vp, nat, l).scale).toBe(FIT);
  ...
  it('a picture that fits stays inside the viewport however it is dragged', () => {
    ...
    expect(panBy(v, 300, 300, vp, wide).ty).toBe(800 - 250);
