...
test.describe('adding to a brand', () => {
  ...
  test('each flow says what pressing its button will actually do', async ({ page }) => {
    ...
    // the product studio saves without generating, and says so where the other two say what they will spend
    await expect(page.locator('.sc-dlg-foot')).toContainText('Nothing generated');
