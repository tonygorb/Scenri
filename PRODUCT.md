...
scenri is the open-source studio for brand-consistent AI visuals. It replaces the "prompt box and slot machine" pattern of generic AI image tools with real art direction: every shot is a node in a tree, edits branch off any node, and a drift-diff shows exactly where a model changed a product when it shouldn't have. Success looks like a designer trusting the tool enough to run an entire client shoot through it without fighting the UI to find their products, presenters, or looks.
...
Generic SaaS dashboard: identical icon+heading+text card grids, gradient-text emphasis, hero-metric tiles (big number / small label / accent gradient), tiny uppercase tracked eyebrows above every section, cream/beige "AI-default" palettes, side-stripe accent borders. This is a working creative tool, not a marketing surface pretending to be one.
...
WCAG AA baseline: body text ≥4.5:1 contrast (including placeholder text), large text ≥3:1, full keyboard navigation, visible focus states, `prefers-reduced-motion` alternative for any animation.
